  ...
  const generateShareImage = async () => {
    ...
    try {
      ...
      const hasValidImage = event.imageUrl && 
        !event.imageUrl.includes('X-Amz-Expires') && // Skip expired AWS URLs
        !event.imageUrl.includes('expired') &&
        event.imageUrl.length > 0
      
      const imageHeight = hasValidImage ? 420 : 0  // Increased from 320 to 420
      let contentStartY = cardY + 48

      if (hasValidImage && imageHeight > 0) {
        ...
        try {
          ...
          // Create array of potential image URLs to try
          const imageUrls = [event.imageUrl]
          
          // If it's an AWS URL with expiration, try to find alternatives
          if (event.imageUrl.includes('amazonaws.com') && event.imageUrl.includes('X-Amz-Expires')) {
            // Try without query parameters as fallback
            const baseUrl = event.imageUrl.split('?')[0]
          ...
          // Try different image URLs and loading methods
          for (const imageUrl of imageUrls) {
            try {
              await new Promise((resolve, reject) => {
                const imgTest = new Image()
                imgTest.onload = () => {
                  console.log(`✅ Event image loaded from: ${imageUrl}`)
                  // Copy successful image to main img
                  img.src = imgTest.src
                  img.onload = () => resolve(img)
                  imageLoaded = true
                }
                imgTest.onerror = () => {
                  console.log(`❌ Failed to load from: ${imageUrl}`)
              ...
              if (imageLoaded) break // Exit loop if we successfully loaded an image
              
            } catch (error) {
              console.log(`⚠️ Continuing to next image URL...`)
              continue
            }
          }
          
          if (!imageLoaded) {
            // Final attempt without CORS
            await new Promise((resolve, reject) => {
              img.onload = () => {
          ...
          if (img.complete && img.naturalWidth > 0) {
            console.log(`🎨 Drawing event image: ${img.naturalWidth}x${img.naturalHeight}`)
            ...
            console.log('✅ Event image drawn successfully in share image')
            
            // Add slight gradient overlay for better text readability
          ...
          console.log('❌ Image loading failed completely, continuing without image:', error)
        }
